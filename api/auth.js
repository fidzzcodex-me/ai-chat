...
function getEnv() {
  ...
  const site = (process.env.SITE_URL || 'https://www.websiteku.dev').replace(/\/$/, '')
...
module.exports = async function handler(req, res) {
  ...
  // Get action from query param ?action=xxx OR from URL path
  let action = '', qs = new URLSearchParams()
  try {
    const u = new URL(req.url, `https://${req.headers.host}`)
    qs = u.searchParams
    // Path: /api/auth/me → action = me
    const parts = u.pathname.split('/').filter(Boolean)
    // parts = ['api','auth','me'] → last part is action
    action = parts[parts.length - 1] || ''
    if (action === 'auth') action = qs.get('action') || ''
  } catch {}
  ...
  try {
    ...
    if (action === 'register' && req.method === 'POST') {
      ...
      const { ok, data } = await sbFetch(url, '/auth/v1/signup', key, {
        method: 'POST', body: JSON.stringify({ email, password, data: { full_name: name || email.split('@')[0] } })
      })
    ...
    if (action === 'callback' && req.method === 'GET') {
      ...
      const { ok, data } = await sbFetch(url, `/auth/v1/token?grant_type=pkce`, key, {
        method: 'POST', body: JSON.stringify({ auth_code: code })
      })
      if (ok && data.access_token) {
        res.setHeader('Location', `${site}/chat#token=${data.access_token}`)
        return res.status(302).end()
      }
      // Fallback: send code to frontend
